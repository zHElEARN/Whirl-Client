...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Whirl.instance.setmgr.getSettingsByMod(imod) != null)
			for (Setting s : Whirl.instance.setmgr.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Whirl.instance.setmgr.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Whirl.instance.clickgui.closeAllSettings();
				this.extended = b;
