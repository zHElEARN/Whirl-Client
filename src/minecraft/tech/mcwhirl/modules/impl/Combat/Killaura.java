...
public class Killaura extends Module {
	...
	public float yaw/* = mc.thePlayer.rotationYaw*/;
	public float pitch/* = mc.thePlayer.rotationPitch*/;
		...
		for (Object e : mc.theWorld.loadedEntityList) {
			if (e instanceof EntityLivingBase) {
				if (e != mc.thePlayer) {
					...
					if (mc.thePlayer.getDistanceToEntity(p) <= 4.2D && p.isEntityAlive() && p.hurtTime < 1 && hitdelay.hasReached(15L) && !(p.getHealth() < 0) 
							&& (checkEntityID(p))) {
						hitdelay.reset();
