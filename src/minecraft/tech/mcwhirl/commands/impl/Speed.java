...
public class Speed extends Command {
	...
	public void execute(String[] args) {
		if (args.length != 1) {
			msgWithPrefix("¡ìcParameter error: ¡ì7$speed ¡ì2<Mode>");
			return;
		}
		String name = args[0];
		if (name.equalsIgnoreCase("Y-Port") || name.equalsIgnoreCase("Timer")) {
			tech.mcwhirl.modules.impl.Movement.Speed.mode = name;
			msgWithPrefix("¡ìaSetted Speed mode to: ¡ì6 <" + name + ">");
		} else if (name.equalsIgnoreCase("help")) {
			msgWithPrefix("¡ìaY-Port, Timer");
		} else {
			msgWithPrefix("¡ìcMode:¡ìa <" + args[0] + "> ¡ìcnot found!");
		}
	}
