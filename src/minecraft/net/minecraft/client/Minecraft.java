...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public Minecraft(GameConfiguration p_i45547_1_)
    {
        ...
        if (p_i45547_1_.field_178742_e.field_178754_a != null)
        {
            ...
            EventManager.register(new Whirl());
        }
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    else
                    {
                    	...
                    	if (var1 == Keyboard.KEY_RIGHT) {
                    		TabGui.callapsed = true;
                    	}
                    	
                    	if (var1 == Keyboard.KEY_LEFT) {
                    		TabGui.callapsed = false;
                    	...
                    	if (var1 == Keyboard.KEY_RETURN) {
                    		if (TabGui.callapsed) {
								...
								for (Module m : Whirl.instance.moduleManager.getModules()) {
									if (m.getCategory() == Category.values()[TabGui.selectedTab]) {
										modules.add(m);
									}
								}
								...
								modules.get(TabGui.selectedModule).toggle();
							}
                    	}
                    	...
                    	if (var1 == Keyboard.KEY_UP) {
							if (!TabGui.callapsed) {
								if (TabGui.selectedTab > 0) {
									...
									TabGui.selectedTab = Category.values().length - 1;
								}
							} else {
								ArrayList<Module> modules = new ArrayList<Module>();
								
								for (Module m : Whirl.instance.moduleManager.getModules()) {
									if (m.getCategory().equals(Category.values()[TabGui.selectedTab])) {
										modules.add(m);
									}
								}
								TabGui.selectedModule--;
								
								if (TabGui.selectedModule == -1) {
									TabGui.selectedModule = modules.toArray().length - 1;
								}
							}
						}
                    	...
                    	if (var1 == Keyboard.KEY_DOWN) {
							if (!TabGui.callapsed) {
								if (TabGui.selectedTab < Category.values().length - 1) {
									TabGui.selectedTab++;
								} else {
								...
								Collections.sort(modules, new Comparator<Module>() {
									@Override
									public int compare(Module m1,Module m2) {
										return Integer.valueOf(Fonts.tabWhirl.getStringWidth(m2.getDisplayName())).compareTo(Integer.valueOf(Fonts.tabWhirl.getStringWidth(m1.getDisplayName())));
									}
								});
								
								TabGui.selectedModule++;
								
								if (TabGui.selectedModule == modules.size()) {
									TabGui.selectedModule = 0;
								}
							}
						}
