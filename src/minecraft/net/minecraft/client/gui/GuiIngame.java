...
import tech.mcwhirl.tabgui.TabGui;
import tech.mcwhirl.ui.Fonts;
import tech.mcwhirl.utils.ColorUtils;
...
public class GuiIngame extends Gui
{
    ...
    public void func_175180_a(float p_175180_1_)
    {
        ...
        if (Whirl.instance.moduleManager.getModuleByName("HUD").isEnabled()) {
            if (Whirl.instance.setmgr.getSettingByName("Module List").getValBoolean()) renderArrayList();
            if (Whirl.instance.setmgr.getSettingByName("TabGUI").getValBoolean()) {
            	renderName();
            	renderVersion();
            	renderTabGui();
            }
            if (Whirl.instance.setmgr.getSettingByName("Hotbar").getValBoolean()) renderHotbar();
        }
    ...
    private void renderArrayList() {
    	...
    	Collections.sort(Whirl.instance.moduleManager.getModules(),new Comparator<Module>() {
    		@Override
    	...
    	for (Module m : Whirl.instance.moduleManager.getModules()) {
    		if (m.isEnabled()) {
    			int xDistance = sr.getScaledWidth() - fr.getStringWidth(m.getDisplayName()) - 2;
    			int width = sr.getScaledWidth();
    			int height = fr.FONT_HEIGHT;
    			fr.drawString(m.getDisplayName(), xDistance, yOffset, m.getColor());
    ...
    private void renderName() {
    	Fonts.logoWhirl.drawString(Whirl.instance.Client_Name, 3, 5, ColorUtils.rainbowEffeckt(200000000L, 1F).getRGB());
    }
    
    private void renderVersion() {
    	Fonts.logoWhirl.drawString(Whirl.instance.Client_Version, 10 + Fonts.logoWhirl.getStringWidth(Whirl.instance.Client_Name), 5, 0xFFFFFF);
    }
    ...
    private void renderHotbar() {
    	...
    	tech.mcwhirl.utils.RenderHelper.drawRect(0, sr.getScaledHeight() - 23, sr.getScaledWidth(), sr.getScaledHeight(), Integer.MIN_VALUE);
    	
    	// Fonts.tabWhirl.drawString("W", 8, sr.getScaledHeight() - 20, ColorUtils.rainbowEffeckt(200000000L, 1F).getRGB());
    	
    	Fonts.smallWhirl.drawString("FPS: ", 3, sr.getScaledHeight() - 23, 0xFF5555);
    	Fonts.smallWhirl.drawString("" + Minecraft.getMinecraft().debugFPS, Fonts.smallWhirl.getStringWidth("FPS: ") + 3, sr.getScaledHeight() - 23, 0xFFFFFF);
    
    	Fonts.smallWhirl.drawString("Ping: ", 3, sr.getScaledHeight() - 13, 0xFF5555);
    	Fonts.smallWhirl.drawString("" + (mc.isSingleplayer() ? "0" : mc.getCurrentServerData().pingToServer), Fonts.smallWhirl.getStringWidth("Ping: ") + 3, sr.getScaledHeight() - 13, 0xFFFFFF);
    }
